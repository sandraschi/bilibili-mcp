...
# PyInstaller spec — produce bilibili-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller bilibili-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    # run_server.py (not __main__.py) switches to --mode http via MCP_PORT/PORT
    # env vars; the Tauri backend.rs spawn sets PORT so the frozen exe must run
    # HTTP mode, not the stdio default __main__.py -> main() falls back to.
    # Using __main__.py here crashes under the windowed subsystem (no console,
    # sys.stdin is None) when mcp.run() tries stdio transport.
    ["run_server.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/bilibili_mcp", "bilibili_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
